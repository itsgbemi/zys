...
const ResignationLetterBuilder: React.FC<ResignationLetterBuilderProps> = ({ 
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const stream = aiService.generateStream(selectedModel, newMessages, "", { type: 'resignation-letter', userProfile });
      let fullText = "";
      ...
      <header className={`p-4 md:p-6 border-b flex items-center justify-between transition-colors sticky top-0 z-10 ${theme === 'dark' ? 'bg-[#191919] border-[#2a2a2a]' : 'bg-white border-[#e2e8f0]'}`}>
        ...
        {activeSession.messages.length > 0 && (
          <button onClick={async () => {
            setIsTyping(true);
            try {
              const result = await aiService.sculpt(selectedModel, `Act as an HR Professional. Draft a firm but gracious exit statement. Data: ${activeSession.messages.map(m => m.content).join('\n')}`);
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8">
        <div className="flex justify-start">
           <div className={`max-w-full text-sm leading-relaxed ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>
              ...
              <MarkdownLite text="Hi there. I'll help you write a professional and graceful resignation. When is your intended last day and who are we addressing?" theme={theme} />
           </div>
        </div>
